...
/// Signature used by [EinblickeDismissible] to indicate that it has been dismissed in
/// the given `direction`.
///
/// Used by [EinblickeDismissible.onDismissed].
typedef DismissDirectionCallback = void Function(
    EinblickeDismissDirection direction);

/// Signature used by [EinblickeDismissible] to give the application an opportunity to
/// confirm or veto a dismiss gesture.
///
/// Used by [EinblickeDismissible.confirmDismiss].
typedef ConfirmDismissCallback = Future<bool?> Function(
    EinblickeDismissDirection direction);

/// Signature used by [EinblickeDismissible] to indicate that the dismissible has been dragged.
///
/// Used by [EinblickeDismissible.onUpdate].
typedef DismissUpdateCallback = void Function(DismissUpdateDetails details);

/// The direction in which a [EinblickeDismissible] can be dismissed.
enum EinblickeDismissDirection {
  /// The [EinblickeDismissible] can be dismissed by dragging either up or down.
  vertical,

  /// The [EinblickeDismissible] can be dismissed by dragging either left or right.
  horizontal,

  /// The [EinblickeDismissible] can be dismissed by dragging in the reverse of the
  /// reading direction (e.g., from right to left in left-to-right languages).
  endToStart,

  /// The [EinblickeDismissible] can be dismissed by dragging in the reading direction
  /// (e.g., from left to right in left-to-right languages).
  startToEnd,

  /// The [EinblickeDismissible] can be dismissed by dragging up only.
  up,

  /// The [EinblickeDismissible] can be dismissed by dragging down only.
  down,

  /// The [EinblickeDismissible] cannot be dismissed by dragging.
  none
}

/// A widget that can be dismissed by dragging in the indicated [direction].
///
/// Dragging or flinging this widget in the [EinblickeDismissDirection] causes the child
...
///
/// {@tool dartpad}
/// This sample shows how you can use the [EinblickeDismissible] widget to
...
class EinblickeDismissible extends StatefulWidget {
  ...
  ///
  /// The [key] argument is required because [EinblickeDismissible]s are commonly used in
  ...
  const EinblickeDismissible({
    ...
    this.onUpdate,
    this.onDismissed,
    this.direction = EinblickeDismissDirection.horizontal,
    this.resizeDuration = const Duration(milliseconds: 300),
    this.dismissThresholds = const <EinblickeDismissDirection, double>{},
  ...
  final EinblickeDismissDirection direction;
  ...
  /// [EinblickeDismissDirection] even if it would be allowed by the [direction]
  /// property.
  ///
  /// See also:
  ///
  ...
  ///    be dismissed.
  final Map<EinblickeDismissDirection, double> dismissThresholds;
...
/// See also:
///
///   * [EinblickeDismissible.onUpdate], which receives this information.
class DismissUpdateDetails {
  ...
  /// that the [EinblickeDismissible] is dragged across the threshold.
...
class _EinblickeDismissibleState extends State<EinblickeDismissible>
  ...
  bool get _directionIsXAxis {
    return widget.direction == EinblickeDismissDirection.horizontal ||
        widget.direction == EinblickeDismissDirection.endToStart ||
        widget.direction == EinblickeDismissDirection.startToEnd;
  }

  EinblickeDismissDirection _extentToDirection(double extent) {
    if (extent == 0.0) {
      return EinblickeDismissDirection.none;
    }
    if (_directionIsXAxis) {
      switch (Directionality.of(context)) {
        case TextDirection.rtl:
          return extent < 0
              ? EinblickeDismissDirection.startToEnd
              : EinblickeDismissDirection.endToStart;
    ...
    return extent > 0
        ? EinblickeDismissDirection.down
        : EinblickeDismissDirection.up;
  }

  EinblickeDismissDirection get _dismissDirection =>
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    switch (widget.direction) {
      case EinblickeDismissDirection.horizontal:
      case EinblickeDismissDirection.vertical:
        _dragExtent += delta;

      case EinblickeDismissDirection.up:
        if (_dragExtent + delta < 0) {
          _dragExtent += delta;
        }

      case EinblickeDismissDirection.down:
        if (_dragExtent + delta > 0) {
          _dragExtent += delta;
        }

      case EinblickeDismissDirection.endToStart:
        switch (Directionality.of(context)) {
          case TextDirection.rtl:
            if (_dragExtent + delta > 0) {
              _dragExtent += delta;
      ...
      case EinblickeDismissDirection.startToEnd:
        switch (Directionality.of(context)) {
          case TextDirection.rtl:
            if (_dragExtent + delta < 0) {
              _dragExtent += delta;
      ...
      case EinblickeDismissDirection.none:
        _dragExtent = 0;
    }
  ...
  _FlingGestureKind _describeFlingGesture(Velocity velocity) {
    ...
    EinblickeDismissDirection flingDirection;
  ...
  Future<bool> _confirmStartResizeAnimation() async {
    if (widget.confirmDismiss != null) {
      _confirming = true;
      final EinblickeDismissDirection direction = _dismissDirection;
      try {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.secondaryBackground != null) {
      final EinblickeDismissDirection direction = _dismissDirection;
      if (direction == EinblickeDismissDirection.endToStart ||
          direction == EinblickeDismissDirection.up) {
    ...
    // cannot be dragged.
    if (widget.direction == EinblickeDismissDirection.none) {
      return content;
    }
